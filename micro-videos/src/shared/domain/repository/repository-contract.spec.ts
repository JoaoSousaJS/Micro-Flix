...
describe('SearchParams Unit Tests', () => {
  ...
  test('per_page prop', () => {
    ...
    arrange.forEach((i) => {
      expect(new SearchParams({
        per_page: i.per_page as any,
      }).per_page).toBe(i.expected);
    });
  });
  ...
  test('sort_dir prop', () => {
    ...
    arrange.forEach((i) => {
      expect(new SearchParams({
        sort_dir: i.sort_dir as any,
        sort: 'field',
      }).sort_dir).toBe(i.expected);
    });
  });
  ...
  test('filter prop', () => {
    ...
    arrange.forEach((i) => {
      expect(new SearchParams({
        filter: i.filter as any,
      }).filter).toBe(i.expected);
    });
  });
});
...
describe('SearchResult Unit Tests', () => {
  ...
  it('should validate last_page prop when total is not a multiple of per_page', () => {
    const result = new SearchResult({
      items: [] as any,
      total: 101,
      current_page: 1,
